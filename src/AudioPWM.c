...
void AudioPWM_Init() {
	...
	// Init audio GPIO
	gpio_set_mode(GPIOB, GPIO_MODE_OUTPUT_50_MHZ, GPIO_CNF_OUTPUT_ALTFN_PUSHPULL, 1 << 13);
	...
	timer_set_oc_value(TIM4, TIM_OC2, PWM_PERIOD/2);
	...
	if(++AudioPWM_SubSampleCount >= AUDIOPWM_SUBSAMPLE_COUNT) {
		...
		if(AudioPWM_SamplingEnabled) {
			...
			if(!AUDIOPWM_BUFFER_EMPTY()) {
				...
				timer_set_oc_value(TIM4, TIM_OC2, AudioPWM_Buffer_Take());
			}
			else
			{
